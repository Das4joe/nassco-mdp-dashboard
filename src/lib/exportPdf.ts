...
// ── Capture Leaflet map SVG paths as canvas image ──────────────
async function captureLeafletMap(containerId: string): Promise<string | null> {
  await wait(800); // let map settle
  ...
  // Grab all SVG layers from Leaflet
  ...
  for (const svgEl of Array.from(svgLayers)) {
    ...
    try {
      ...
      // Inline all computed styles on paths
...
function drawLabel(
  ...
  const padX     = 8;
  const padY     = 5;
  const totalH   = mainSize + subSize + padY * 2 + 4;
  ...
  ctx.beginPath();
  ctx.moveTo(rx + r, ry);
  ctx.lineTo(rx + totalW - r, ry);
  ctx.quadraticCurveTo(rx + totalW, ry, rx + totalW, ry + r);
  ctx.lineTo(rx + totalW, ry + totalH - r);
  ctx.quadraticCurveTo(rx + totalW, ry + totalH, rx + totalW - r, ry + totalH);
  ctx.lineTo(rx + r, ry + totalH);
  ctx.quadraticCurveTo(rx, ry + totalH, rx, ry + totalH - r);
  ctx.lineTo(rx, ry + r);
  ...
  ctx.fillText(subtext, x, ry + padY + mainSize + 3);
...
function buildPDF(options: {
  ...
  labels:       { name: string; sub: string; color: string; xPct: number; yPct: number }[];
  tableHeaders: string[];
  tableRows:    string[][];
  date:         string;
  ...
  const {
    title, subtitle, mapCanvas, labels,
    tableHeaders, tableRows, date,
  ...
  if (mapCanvas) {
    ...
    // Draw labels directly on the PDF over the map
    if (labels.length > 0) {
  ...
  if (tableRows.length > 0) {
    ...
    tableRows.forEach((row, ri) => {
      if (curY > pageH - 18) { pdf.addPage(); curY = M; }
    ...
    if (curY < pageH - 18) {
      ...
      const totalHH = tableRows.reduce(
        (s, r) => s + parseInt(String(r[1]).replace(/,/g, "") || "0"), 0
      );
      if (colCount > 1) pdf.text(totalHH.toLocaleString(), M + colW + 3, curY + 5.5);
    }
  }
...
// ── Label positions for each intervention state on Nigeria map ──
const STATE_MAP_POSITIONS: Record<string, { xPct: number; yPct: number }> = {
  Sokoto: { xPct: 0.30, yPct: 0.18 },
  Benue:  { xPct: 0.60, yPct: 0.60 },
  Oyo:    { xPct: 0.28, yPct: 0.65 },
  Abia:   { xPct: 0.58, yPct: 0.80 },
};
...
export async function exportNigeriaMap(
  ...
  const pdf = buildPDF({
    ...
    mapCanvas: mapImg,
    labels,
    tableHeaders: ["State", "Enumerated HHs", "LGAs Covered", "Communities", "% of Total"],
  ...
  pdf.save(`NASSCO-Nigeria-Overview-${new Date().toISOString().slice(0, 10)}.pdf`);
}
...
export async function exportStateMap(
  ...
  // LGA labels — distribute across map area
